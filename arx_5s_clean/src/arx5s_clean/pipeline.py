...
from arx5s_clean.config import INPUT_COLS, ExperimentConfig
from arx5s_clean.data.collection.step_00_schema import normalize_model_columns
...
def _runtime_artifact(
    ...
    return {
        ...
        "clip_policy": {
            "type": "physical_soil_range",
            "low": max(0.0, cfg.soil_low_sp - cfg.soil_clip_margin),
            "high": min(100.0, cfg.soil_high_sp + cfg.soil_clip_margin),
        },
        "feature_policy": {
            "source": "observed sensors, actuator states, and time features",
            "created_features": [
                "Light_log",
                "Temp_x_Humidity",
                "Temp_x_Light",
...
def run_pipeline(project_root: Path, cfg: ExperimentConfig, grid: str) -> dict[str, Any]:
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"{data_path} does not exist. Run scripts/01_build_data_from_collection.py before training."
        )
    ...
    raw.loc[:, MODEL_DATA_COLS].to_csv(data_path, index=False)
