...
# Kiểm tra cấu trúc file raw có đủ 8 cột bắt buộc.
...
# Chuẩn hóa Timestamp về lưới 5 giây và sắp xếp theo thời gian.
...
# Gộp các dòng trùng Timestamp trong cùng một file raw.
...
# Tạo lại đầy đủ mốc thời gian 5 giây; mốc thiếu sẽ tạm là NaN.
def reindex_to_5s_grid(df: pd.DataFrame) -> pd.DataFrame:
    ...
    gridded = df.set_index("Timestamp").reindex(grid) # set nan cho timestamp thiếu
    gridded.index.name = "Timestamp"
    return gridded


# Ép sensor và actuator về dạng số
...
# Xử lý missing và outlier sensor bằng nội suy theo thời gian.
def fill_sensor_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col, (low, high) in SENSOR_RANGES.items():
        ...
        out[col] = out[col].interpolate(method="time", limit=12, limit_direction="both") # lấy nội suy ở 12 điểm gần nhất, ỡ giữa
        out[col] = out[col].ffill().bfill() # ở đầu cuối thì lấy giá trị gần nhất
        out[col] = out[col].clip(low, high)
    return out


# Làm mềm các điểm Soil_Moisture nhảy bất thường so với vùng lân cận.
def smooth_soil_moisture(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    soil = out["Soil_Moisture"].astype(float)

    local_median = soil.rolling(window=5, center=True, min_periods=1).median() #trung vị
    spike = (soil - local_median).abs() > 5.0 # so sánh
    out.loc[spike, "Soil_Moisture"] = np.nan # coi như là missing
...
# Điền trạng thái thiết bị bị thiếu
def fill_actuator_missing(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    for col in ACTUATOR_COLS:
        invalid = ~out[col].isin([0.0, 1.0]) & out[col].notna()
        out.loc[invalid, col] = np.nan # coi như là missing
        out[col] = out[col].ffill(limit=12).bfill(limit=2).fillna(0.0) # vẫn k biết thì coi như là tắt
        out[col] = (out[col] >= 0.5).astype(float)
    return out


# Clean một file raw trong _01_data.
...
# Clean toàn bộ file raw và ghi kết quả sang _02_clean_data.
