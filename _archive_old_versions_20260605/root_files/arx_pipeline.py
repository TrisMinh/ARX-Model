...
def residual_diagnostics(
    ...
    for col in model_config.input_cols:
        ...
        r_vals = residuals[:n_min]
        # Guard against near-constant vectors that would produce NaN correlation.
        if len(r_vals) == 0 or np.std(r_vals) < 1e-12 or np.std(u_vals) < 1e-12:
            cc = 0.0
        else:
            cc = float(np.corrcoef(r_vals, u_vals)[0, 1])
