...
NOTEBOOK_PATH = Path("ARX_Model_Version") / "ARX_Baseline_V6.ipynb"
...
V6 giữ nguyên V5 và thêm clip free-run để kiểm soát drift:

- `ARX(2,2,1)`
- 6 input gốc + 10 feature engineer
- Ridge
- không Lasso
- clip free-run theo Q1%-Q99% của train
...
clip_low_real = float(df_train_aug["Soil_Moisture"].quantile(CLIP_QUANTILES[0]))
clip_high_real = float(df_train_aug["Soil_Moisture"].quantile(CLIP_QUANTILES[1]))
y_scale = scale_stats["Soil_Moisture"]
clip_low_scaled = (clip_low_real - y_scale["mean"]) / y_scale["std"]
clip_high_scaled = (clip_high_real - y_scale["mean"]) / y_scale["std"]
CLIP_BOUNDS_REAL = (clip_low_real, clip_high_real)
CLIP_BOUNDS_SCALED = (float(clip_low_scaled), float(clip_high_scaled))

V6_CONFIG = ModelConfig(
    na=V6_CONFIG_NO_CLIP.na,
    nb=V6_CONFIG_NO_CLIP.nb,
    nk=V6_CONFIG_NO_CLIP.nk,
    include_intercept=V6_CONFIG_NO_CLIP.include_intercept,
    input_cols=V6_CONFIG_NO_CLIP.input_cols,
    output_col=V6_CONFIG_NO_CLIP.output_col,
    simulation_clip=CLIP_BOUNDS_SCALED,
)

display(pd.DataFrame(scale_stats).T)
pd.Series({
...
"""
        ...
        code(
            """
X_train_z, y_train_z = build_regression_matrix(df_train_z, V6_CONFIG)
theta_ols_z, cov_ols_z, sigma2_ols_z = estimate_ols(X_train_z, y_train_z)
true_theta = build_true_theta(true_params, V6_CONFIG)
...
def estimate_ridge(x_mat: np.ndarray, y_vec: np.ndarray, alpha: float, penalize_intercept: bool = False) -> np.ndarray:
    ...
    if not penalize_intercept and V6_CONFIG.include_intercept:
        penalty[-1, -1] = 0.0
...
def real_unit_metrics_for_theta(theta: np.ndarray, split_name: str, df_z: pd.DataFrame, mode: str) -> dict[str, float]:
    ev = evaluate_slice(split_name, df_z, theta, V6_CONFIG, true_theta=true_theta, n_step=12)
    arrays = ev["arrays"]
    if mode == "1step":
...
"""
        ...
        code(
            """
train_eval_z = evaluate_slice("Train", df_train_z, theta_hat_z, V6_CONFIG, true_theta=true_theta, n_step=12)
val_eval_z = evaluate_slice("Validation", df_val_z, theta_hat_z, V6_CONFIG, true_theta=true_theta, n_step=12)
test_eval_z = evaluate_slice("Test", df_test_z, theta_hat_z, V6_CONFIG, true_theta=true_theta, n_step=12)
...
"""
        ...
        md("## 10. So sánh với V1/V2/V3/V4/V5 artifact"),
        code(
            """
v1_path_candidates = [
...
v5_path_candidates = [
    VERSION_DIR / "arx_baseline_v5.json",
    PROJECT_ROOT / "arx_baseline_v5.json",
]
v1_artifact = None
...
for path in v4_path_candidates:
    if path.exists():
        ...
        break
v5_artifact = None
for path in v5_path_candidates:
    if path.exists():
        with path.open("r", encoding="utf-8") as f:
...
if v1_artifact is None:
    ...
    if v5_artifact is not None:
        rows.append(
            {
                "version": f"V5 augmented Ridge alpha={v5_artifact['regularization']['best_alpha']:g}",
                "val_FIT_sim": v5_artifact["metrics"]["validation"]["fit_sim"],
                "test_FIT_sim": v5_artifact["metrics"]["test"]["fit_sim"],
            }
        )
    rows.append(
        {
            "version": f"V6 augmented + clip Ridge alpha={best_alpha:g}",
...
for ax, pred_df, title in [
    ...
    ax.plot(pred_df["idx"], pred_df["y_pred_sim"], label="Free-run V6", linewidth=1.0)
    ax.set_title(title)
...
"""
        ...
        code(
            """
ar_roots = compute_ar_roots(theta_hat_z, V6_CONFIG)
pd.DataFrame(ar_roots)
"""
        ),
        md("## 13. Lưu artifact V6"),
        code(
            """
...
artifact = {
    "model_type": "ARX",
    "version": "v6_augmented_zscore_intercept_ridge_clip",
    "notes": "ARX(2,2,1), 16 augmented inputs, Ridge alpha search, z-score normalization from train stats, include_intercept=True, free-run clip from train Q1%-Q99%.",
    "data_source": data_source,
    "data_config": DATA_CONFIG.__dict__,
    "split_config": SPLIT_CONFIG.__dict__,
    "model_config": V6_CONFIG.__dict__,
    "feature_engineering": {
        "enabled": True,
    ...
    "param_names": V6_CONFIG.param_names,
    ...
    "comparison_vs_v1_v2_v3_v4_v5": comparison_df.to_dict(orient="records") if not comparison_df.empty else [],
}

VERSION_DIR.mkdir(exist_ok=True)
output_path = VERSION_DIR / "arx_baseline_v6.json"
...
V6 chỉ trả lời một câu hỏi: thêm clip free-run sau feature engineering có kiểm soát drift và cải thiện `FIT_sim` không.

Nếu V6 cải thiện rõ, điều đó cho thấy feature engineering cần đi cùng ràng buộc miền giá trị khi dùng `ARX(2,2,1)`.
"""
        ),
    ]
